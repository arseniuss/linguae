...
public class TrainingSetupViewModel extends AndroidViewModel {
    ...
    private final MutableLiveData<List<TrainingConfigViewModel>> _trainingConfig = new MutableLiveData<>(
            new ArrayList<>());
    ...
    public void Load(String trainingId) {
        Disposable d = _trainingDataAccess.GetTrainingConfig(trainingId)
                ...
                .subscribe(trainingConfigs -> {
                    Map<TaskType, List<TrainingConfig>> collect = trainingConfigs.stream()
                            .collect(groupingBy(TrainingConfig::getType));
    ...
    public static class TrainingConfigViewModel extends BaseObservable {
        ...
        private final MutableLiveData<List<SelectionViewModel>> _descriptions = new MutableLiveData<>(
                new ArrayList<>());
