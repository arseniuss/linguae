...
import lv.id.arseniuss.linguae.app.databinding.ActivitySessionResultBinding;
import lv.id.arseniuss.linguae.app.db.entities.SessionResultWithTaskResults;
import lv.id.arseniuss.linguae.app.viewmodel.SessionResultViewModel;
...
public class SessionResultActivity extends AppCompatActivity {
    ...
    private void onLoaded() {
        List<PieEntry> pieEntryList = _model.GetTaskResults()
                .stream()
                .collect(Collectors.groupingBy(taskResult -> taskResult.Type))
                .values()
                .stream()
