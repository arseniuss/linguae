...
public abstract class TrainingDataAccess {
    @Query("SELECT t.*, COUNT(tt.training_id) AS task_count, COUNT(tc.training_id) AS category_count FROM training t " +
           "LEFT JOIN training_task tt ON t.id = tt.training_id OR t.id IS NULL OR t.id = '' LEFT JOIN " +
           "training_category tc ON t.id = tc.training_id OR t.id IS NULL OR t.id = '' GROUP by t.id")
